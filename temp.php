...
$sqlChart_Q=("SELECT name,unix_timestamp(timestamp) FROM someTable WHERE id > 2500 ORDER BY unix_timestamp(timestamp) ASC");
   $sqlChart_R=mysql_query($sqlChart_Q);
   while($arrChart_ROW=mysql_fetch_row($sqlChart_R)) {
      $time=(int)$arrChart_ROW[1]*1000;
	  $name=(float)$arrChart_ROW[0]; 
      $arrChart[]=array($time, $name);
   }
